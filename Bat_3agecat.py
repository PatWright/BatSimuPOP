...
def demoModel(gen, pop):
    ...
    print(f"{gen} young/adult/old sp0 : {pop.subPopSize([0, 'age < 3'])} /{pop.subPopSize([0, '3 <= age < 9'])} / {pop.subPopSize([0, '9 <= age < 17'])}" +
...
pop1.evolve(
    ...
    matingScheme=sim.HeteroMating([
        ...
        sim.RandomMating(ops=[
            ...
            subPops=[(sim.ALL_AVAIL,'3 <= age < 9'),(sim.ALL_AVAIL,'9 <= age < 17')],
            weight=-0.1),
        # individuals with age < 17 will be kept, but might be removed due to
        # population size decline
        sim.CloneMating(subPops=[(sim.ALL_AVAIL, 'age < 3'), (sim.ALL_AVAIL, '3 <= age < 9'),(sim.ALL_AVAIL, '9 <= age < 17')]),
        ],
